...
function toggleLiabilityRisk(key,name,limit,methodNote){
  ...
  addLiabilityRisk(key,name,limit,methodNote);
}
function riskHelpBox(text){
  return text ? `<details class="risk-help"><summary>Metodická nápověda</summary><p>${esc(text)}</p></details>` : '';
}
function tabLiability(){
  ...
  <div class="section-soft"><div class="section-head"><div><h3>Vybraná rizika odpovědnosti</h3><p class="muted">Interní metodika je pouze v detailu rizika. Do poptávky jde jen limit, spoluúčast a pole „Specifikace / doplnění pro pojišťovnu“.</p></div><div class="tools"><button class="btn secondary" onclick="addCustomRisk()">+ Vlastní riziko</button><button class="btn danger" onclick="clearAllLiabilityRisks()">Odebrat všechna rizika</button></div></div><div class="table-wrap"><table class="pro-table risk-ux-table"><thead><tr><th>Produkt / riziko</th><th>Požadovaný limit</th><th>Spoluúčast</th><th class="wide-spec">Specifikace / doplnění pro pojišťovnu</th><th></th></tr></thead><tbody>${chosen.length?chosen.map((r,i)=>{const idx=state.risks.indexOf(r);return `<tr><td class="risk-product-cell"><textarea class="fit risk-name" onchange="state.risks[${idx}].name=this.value">${esc(r.name)}</textarea>${riskHelpBox(riskMethodNote(r))}<small class="muted">Interní kód: ${esc(r.risk_key)}</small></td><td><textarea class="fit" onchange="state.risks[${idx}].requested_limit=this.value">${esc(r.requested_limit||'')}</textarea></td><td><textarea class="fit" onchange="state.risks[${idx}].deductible=this.value">${esc(r.deductible||'')}</textarea></td><td><textarea class="fit tall spec-field" onchange="state.risks[${idx}].specification=this.value" placeholder="Text určený pro pojišťovnu / poptávku">${esc(riskSpecification(r))}</textarea></td><td><button class="btn danger" onclick="removeRisk(${idx})">Smazat</button></td></tr>`}).join(''):'<tr><td colspan="5" class="muted">Zatím není vybrané žádné riziko odpovědnosti.</td></tr>'}</tbody></table></div></div>
...
/* === BRH 3.2.0 – Professional Risk Workflow & Textation Engine === */
...
function riskMethodTooltip(r){
  const txt=riskMethodNote(r)||'K tomuto riziku zatím není doplněna interní metodika. Tato nápověda je určena pouze poradci a nepropíše se do poptávky ani do klientského výstupu.';
  return `<span class="risk-hover-tip" role="tooltip">${esc(txt)}</span>`;
}
function riskProfessionalRow(r,idx){
  return `<div class="risk-pro-row">
    <div class="risk-pro-cell risk-pro-product">
      <label>Produkt / riziko</label>
      <div class="risk-name-hover" tabindex="0">
        <strong>${esc(r.name||r.label||'Riziko')}</strong>
        <span class="info-dot">i</span>
        ${riskMethodTooltip(r)}
      </div>
      <input class="risk-name-edit" value="${esc(r.name||r.label||'')}" onchange="state.risks[${idx}].name=this.value" aria-label="Název rizika">
    </div>
    <div class="risk-pro-cell">
      <label>Požadovaný limit</label>
      <textarea class="fit" onchange="state.risks[${idx}].requested_limit=this.value" placeholder="např. 10 000 000 Kč">${esc(r.requested_limit||r.limit||'')}</textarea>
    </div>
    <div class="risk-pro-cell">
      <label>Spoluúčast</label>
      <textarea class="fit" onchange="state.risks[${idx}].deductible=this.value" placeholder="např. 10 000 Kč">${esc(r.deductible||'')}</textarea>
    </div>
    <div class="risk-pro-cell risk-pro-spec">
      <label>Specifikace / doplnění pro pojišťovnu</label>
      <textarea class="fit tall spec-field" onchange="state.risks[${idx}].specification=this.value" placeholder="Text určený do poptávky, nabídky a komunikace s pojišťovnou. Interní metodika se sem nepropíše.">${esc(riskSpecification(r))}</textarea>
    </div>
    <div class="risk-pro-cell risk-pro-actions">
      <button class="btn danger" onclick="removeRisk(${idx})">Smazat</button>
    </div>
  </div>`;
}
function riskProfessionalTable(rows,emptyText){
  return `<div class="risk-pro-table"><div class="risk-pro-head"><div>Produkt / riziko</div><div>Požadovaný limit</div><div>Spoluúčast</div><div>Specifikace / doplnění pro pojišťovnu</div><div>Akce</div></div>${rows || `<div class="risk-pro-empty">${esc(emptyText)}</div>`}</div>`;
}
function tabLiability(){
  const selected=liabilitySelectedSet();
  const riskCards=LIABILITY_RISKS.map(r=>`<button class="risk-tile ${selected.has(r[0])?'selected':''}" title="${esc(r[3])}" onclick="toggleLiabilityRisk('${esc(r[0])}','${esc(r[1])}','${esc(r[2])}','${esc(r[3])}')"><span>${selected.has(r[0])?'✓ Odebrat: ':'+ '}${esc(r[1])}</span><small>Doporučený limit: ${esc(r[2])}</small><em>${esc(r[3])}</em></button>`).join('');
  ...
  const chosen=(state.risks||[]).filter(r=>LIABILITY_RISKS.some(x=>x[0]===r.risk_key)||String(r.risk_key||'').startsWith('CUSTOM'));
  const riskRows=chosen.map(r=>riskProfessionalRow(r,state.risks.indexOf(r))).join('');
  ...
  <div class="section-soft"><div class="section-head"><div><h3>Profesionální katalog rizik odpovědnosti</h3><p class="muted">Riziko přidáte kliknutím. Pokud je už vybrané, dalším kliknutím ho z poptávky odeberete. Metodika se zobrazuje jako nápověda pro poradce.</p></div><div class="tools"><button class="btn primary" onclick="addAllLiabilityRisks()">+ Přidat celou sadu</button><button class="btn danger" onclick="clearAllLiabilityRisks()">Odebrat všechna rizika</button></div></div><div class="risk-catalog-grid compact">${riskCards}</div></div>
  <div class="section-soft"><div class="section-head"><div><h3>Vybraná rizika odpovědnosti</h3><p class="muted">Najeďte myší na název rizika pro interní nápovědu. Tato interní metodika se nikam neexportuje.</p></div><button class="btn secondary" onclick="addCustomRisk()">+ Vlastní riziko</button></div>${riskProfessionalTable(riskRows,'Zatím není vybrané žádné riziko odpovědnosti.')}</div>
...
function tabRisks(){
  const options=riskOptions(); const suggested=suggestedRisks();
  const rows=(state.risks||[]).map((r,i)=>riskProfessionalRow(r,i)).join('');
  return `<p class="eyebrow">4. Ostatní rizika a požadavky klienta</p><h2>Vstupní dotazník rizik</h2><p class="muted">Každé riziko má oddělenou interní metodiku a externí specifikaci pro pojišťovnu.</p><div class="section-soft"><h3>Doporučená rizika podle činnosti</h3>${suggested.length?`<div class="chip-row">${suggested.map(r=>`<button class="chip" title="${esc(r.note||'')}" onclick="addRiskByKey('${esc(r.risk_key)}')">+ ${esc(r.risk_name||r.name||r.risk_key)}</button>`).join('')}</div>`:'<div class="info-box">Vyplňte typ činnosti na kartě klienta nebo použijte ruční výběr z knihovny rizik.</div>'}</div><div class="tools"><select id="addRiskSelect"><option value="">Vyberte riziko z knihovny</option>${options}</select><button class="btn primary" onclick="addRiskFromSelect()">+ Přidat riziko</button><button class="btn secondary" onclick="addDefaultRisks()">+ Doporučená sada</button><button class="btn secondary" onclick="addCustomRisk()">+ Vlastní riziko</button></div><div class="section-soft"><div class="section-head"><div><h3>Všechna rizika případu</h3><p class="muted">Najeďte myší na název rizika pro interní metodiku. Do poptávky se propisuje pouze specifikace pro pojišťovnu.</p></div><button class="btn danger" onclick="clearAllLiabilityRisks()">Odebrat všechna rizika odpovědnosti</button></div>${riskProfessionalTable(rows,'Zatím není doplněné žádné riziko.')}</div><div class="tools"><button class="btn primary" onclick="readCurrentTab();saveCase()">Uložit rizika</button><button class="btn secondary" onclick="currentTab='insurers';renderWorkspace()">Pokračovat na pojišťovny</button></div>`;
}
function insurerRequestHtml(code, forPrint=false){
  const ins=insurerByCode(code); const req=ensureInsurerRequest(code); const rows=requestDataRows();
