...
/* ==========================================================
   Business Risk Hub 4.6.1 – Release Identity Fix & Visible Build Check
...
/* ==========================================================
   Business Risk Hub 4.6.1 – Advisor Professional Cards Workflow SAFE
...
(function(){
  ...
  function makeCaseRisk410(r){
    return {
      ...
      source: safeText(r.source || 'Advisor Professional Cards Workflow SAFE 4.6.1')
    };
  }
  ...
  window.addAllLiabilityAgreements = addAllLiabilityAgreements = function(){
    ...
    agreement410List().forEach(a=>{ const k=safeText(a.title).toLowerCase(); if(k && !existing.has(k)) state.liability_agreements.push({title:a.title, limit:a.limit||'', text:a.text||'', source:a.source||'Advisor Professional Cards Workflow SAFE 4.6.1'}); });
  ...
  window.tabLiability = tabLiability = function(){
    ...
    const agre=(state.liability_agreements||[]).map((a,i)=>`<tr><td><textarea class="fit" onchange="state.liability_agreements[${i}].title=this.value">${esc(a.title||'')}</textarea></td><td><textarea class="fit" onchange="state.liability_agreements[${i}].limit=this.value" placeholder="limit / sublimit">${esc(a.limit||'')}</textarea></td><td><textarea class="fit tall" onchange="state.liability_agreements[${i}].text=this.value">${esc(a.text||'')}</textarea></td><td><button class="btn danger" onclick="removeLiabilityAgreement(${i})">Smazat</button></td></tr>`).join('');
    return `<p class="eyebrow">3. Modul pojištění odpovědnosti PROFI · 4.6.1</p>
...
/* =====================================================================
   BRH 4.6.1 – Advisor Professional Cards Workflow SAFE
  ...
  window.renderAdmin = function(){
    ...
    box.innerHTML = `
      <div class="admin-safe-head">
        <div>
          <p class="eyebrow">Administrace · 4.6.1 SAFE</p>
...
(function(){
  ...
  window.brh431OpenProfessionalCards = function(kind){
    ...
    target.innerHTML =
      '<section class="hero431"><div><p class="eyebrow">'+(isInsurance?'2 · Karta pro pojištění':'1 · Karta klienta')+'</p><h2>'+(isInsurance?'Karta pro pojištění':'Karta klienta')+'</h2><p>Bezpečný režim karet 4.6.1. Hlavní workflow ani tlačítka nejsou přepsána.</p></div></section>'+
...
/* BRH452_EMERGENCY_LAYOUT_SAFE
   Stabilizační release po chybě 4.6.1.
   Záměrně NEPŘIDÁVÁ žádný tab controller ani globální click listener.
   Zachovává původní funkční workflow moduly.
*/
window.BRH_RENDER_VERSION = '452';
window.BRH_WORKFLOW_SAFE = true;


/* BRH461_CLICK_RESTORE_SAFE
   Stabilizační balík: žádný nový tab controller, žádný globální click override.
   Cíl: obnovit funkční klikání a původní workflow.
*/
window.BRH_RENDER_VERSION = '461';
window.BRH_CLICK_RESTORE_SAFE = true;
